...
> **모델이 ‘누구의 목소리인가’보다 ‘누구와 닮았는가’를 이해하는가?**
> 아래의 세 지표는 임베딩 품질·유사도 신뢰도·일반화 성능을 함께 보여줍니다.
...
> 🟦 = 성능 달성도 (●: 0.2 단위)
> 📊 시각적으로 표현하면, **학습 데이터에서는 완성도 높고,
> 실제 테스트 환경에서는 절반 정도의 안정성을 확보한 상태.**
>
> 이는 테스트 데이터와 학습 데이터가 '일반인'과 '아티스트'라는 점에 비추어볼 때, 타당한 결과라고 판단했습니다. 모델은 타당한 견고함을 갖추면서도 새로운 데이터에 대해서 유연하게 판단할 수 있기 때문입니다.
...
* **Silhouette 0.586** → 래퍼별 음성 스타일이 뚜렷이 분리되어,
  모델이 “누가 누구와 닮았는가”를 구별할 수 있음.
* **Train Precision@5 0.815** → 임베딩 유사도 기반 검색이 매우 정확.
  내부 일관성이 높고, 학습된 스타일 구조가 안정적으로 형성됨.
* **Test Precision@5 0.467** → 새로운 랩 클립에서는 유사도 유지율이 약 46.7%.
  이는 **발성/템포/믹싱 변조에 대한 일반화 한계**를 의미하지만,
  실제 창작 입력(지망생 랩)에서도 일정 수준의 유효한 추천이 가능함을 보여줌.
Softmax Accuracy (0.778) → 분류 정확도 중심, “누구인가”를 맞히는 능력

Cosine Accuracy (0.880) → 임베딩 구조 중심, “누구와 닮았는가”를 구별하는 능력

두 지표의 차이는 모델이 거리 기반 표현 학습에 성공했다는 증거야.
---

### ✍️ 포트폴리오용 문장 예시

> “Silhouette 0.586, Precision@5 0.815(Train), 0.467(Test)으로
> 모델은 스타일 군집의 구조적 분리에는 성공했으며,
> 새로운 입력에서도 약 47% 수준의 유사도 일관성을 보였다.
> 이는 모델이 ‘스타일의 경계’를 인식하고 있음을 보여주는 정량적 증거다.”
